...
begin
  ...
  capacity_wait := p_transient and p_error_code in (
    'groq_free_rate_limit', 'gemini_free_rate_limit', 'openrouter_free_rate_limit',
    'cloudflare_ai_unavailable', 'cloudflare_daily_quota', 'groq_unavailable',
    'gemini_unavailable', 'openrouter_unavailable', 'provider_timeout'
  );
  will_retry := p_transient and (target.attempt_count < target.max_attempts or capacity_wait);
  next_status := case when will_retry then 'retry_wait'::public.job_status else 'failed'::public.job_status end;
  retry_seconds := least(
    greatest(
      coalesce(p_retry_delay_seconds, least(5 * (2 ^ greatest(target.attempt_count - 1, 0)), 300)::integer),
      case when capacity_wait then 15 else 5 end
    ),
    case when p_error_code = 'cloudflare_daily_quota' then 86400 when capacity_wait then 60 else 86400 end
  );

  update public.processing_jobs
  set status = next_status,
      stage = case when will_retry then 'backoff' else 'failed' end,
      attempt_count = case when capacity_wait and target.attempt_count >= target.max_attempts then 0 else attempt_count end,
  ...
  set status = case when will_retry then 'queued'::public.class_status else 'failed'::public.class_status end,
      current_stage = case when will_retry then 'Aguardando capacidade; retomada automática' else 'Etapa com falha' end,
      error_message = left(p_public_message, 500)
